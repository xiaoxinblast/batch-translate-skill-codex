...
- **新文件启动**：尚未存在与该源文件相符的受管任务状态。每次都必须先检查工具包远端更新，再扫描资料、编译资源或执行 `init`。先确认 `batch_translate/` 工作树干净且 origin 受信任，获取上游引用；上游领先时只能以快进方式更新。工作树有改动、分支已分叉或更新校验失败时停止并报告，不得重置、覆盖或删除本地数据。
- **中断后续跑**：已有任务状态与源文件 SHA-256 一致。不要拉取、安装依赖或更新工具包，只执行下列本地环境、工具包和角色检查后续跑。若状态缺失或源文件哈希不匹配，按新文件启动处理。
...
新文件的更新检查确认有可用上游提交时，运行工具包自己的更新流程或仅快进合并，并重新执行以上检查和相应测试；不得删除 `data/`、`exports/` 或本地改动，也不得使用强制重置。中断后续跑仅在工具包不存在、上述检查失败、角色未同步或用户明确要求更新时，才安全更新工具包、安装依赖并同步角色。
